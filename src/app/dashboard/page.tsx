...
export default function DashboardPage() {
  ...
  const postsQuery = useMemo(() => db ? query(collection(db, 'forumPosts'), orderBy('comments', 'desc'), limit(3)) : null, []);
  ...
  const getPostAuthor = (authorId: string) => users?.find(u => u.id === authorId);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className='lg:col-span-2 space-y-6'>
            ...
            {feedPosts?.map((post) => {
              ...
              return (
                <Card key={post.id} className="transition-shadow duration-300 hover:shadow-lg hover:shadow-primary/10">
                    <CardHeader>
